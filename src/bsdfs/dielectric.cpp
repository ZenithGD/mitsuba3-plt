 ...
 * - specular_transmittance
   - |spectrum| or |texture|
   - Optional factor that can be used to modulate the specular transmission
component. Note that for physical realism, this parameter should never be
touched. (Default: 1.0)
   - |exposed|, |differentiable|
...
.. subfigure::
../../resources/data/docs/images/render/bsdf_dielectric_diamond.jpg :caption:
...
In this model, the microscopic structure of the surface is assumed to be
perfectly smooth, resulting in a degenerate BSDF described by a Dirac delta
distribution. This means that for any given incoming ray of light, the model
always scatters into a discrete set of directions, as opposed to a continuum.
For a similar model that instead describes a rough surface microstructure, take
a look at the :ref:`roughdielectric <bsdf-roughdielectric>` plugin.
...
An example of how one might describe a slightly absorbing piece of glass is
shown below:
...
polarized implementation of the underlying Fresnel equations that quantify the
reflectance and transmission.

Instead of specifying numerical values for the indices of refraction, Mitsuba 3
comes with a list of presets that can be specified with the
:paramtype:`material` parameter:

.. figtable::
    :label: ior-table-list
    :caption: This table lists all supported material names
       along with along with their associated index of refraction at standard
conditions. These material names can be used with the plugins :ref:`dielectric
<bsdf-dielectric>`, :ref:`roughdielectric <bsdf-roughdielectric>`, :ref:`plastic
<bsdf-plastic>` , as well as :ref:`roughplastic <bsdf-roughplastic>`. :alt: List
table
...
public:
    ...
    GeneralizedRadiance<Float, Spectrum> wbsdf_weight(const BSDFContext &ctx,
        ...
        Float wpdf = 1.f;
        if (likely(has_reflection && has_transmission)) {
            selected_r = dr::dot(si.n, wo) < 0.0f;
            ...
            if (has_reflection || has_transmission) {
                ...
                return GeneralizedRadiance3f(1.0f);
            }
        }
        Mask selected_t = !selected_r && active;
        ...
        if constexpr (is_polarized_v<Spectrum>) {
            ...
            if (likely(has_reflection && has_transmission)) {
                weight = dr::select(selected_r, R, T) / wpdf;
            } else if (has_reflection || has_transmission) {
                weight = has_reflection ? R : T;
                wpdf = 1.f;
            }
