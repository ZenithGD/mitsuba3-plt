...
MI_PY_EXPORT(SampleSolve)
{
    ...
    auto it = nb::class_<PLTSamplePhaseData3f>(m, "PLTSamplePhaseData3f", D(PLTSamplePhaseData3f))
        ...
        .def_field(PLTSamplePhaseData3f, coherence, "coherence")
        .def_field(PLTSamplePhaseData3f, sampling_wavelengths, "sampling_wavelengths")
        .def(nb::init<BSDFSample3f, Vector2i, Vector3f, Coherence3f, UnpolarizedSpectrum>());

    MI_PY_DRJIT_STRUCT(it, PLTSamplePhaseData3f, bsdf_sample, diffraction_lobe, coherence, sampling_wavelengths);
}