...
MI_VARIANT class DirectionalSpreadEmitter final : public Emitter<Float, Spectrum> {
public:
    MI_IMPORT_BASE(Emitter, m_flags, m_to_world, m_needs_sample_3)
    MI_IMPORT_TYPES(Scene, Texture)

    DirectionalSpreadEmitter(const Properties &props) : Base(props) {
        ...
        m_spread_angle = props.get<ScalarFloat>("spread_angle", 0.0);
    ...
    std::pair<Ray3f, Spectrum> sample_ray(Float time, Float wavelength_sample,
        ...
        // 2. Sample point for angular spread
        Point2f angular = warp::square_to_uniform_disk(direction_sample) * dr::sin(m_spread_angle); 

        // 3. "Sample" directional component (fixed, no actual sampling required)
        const auto trafo = m_to_world.value();
        Vector3f d_global = trafo.transform_affine(Vector3f{ 0.0f, 0.0f, 1.f });
        Vector3f d_offset = trafo.transform_affine(Vector3f{angular.x(), angular.y(), 0.0f});
        ...
        return { Ray3f(origin, d_global + d_offset, time, wavelengths),
    ...
    sample_direction(const Interaction3f &it, const Point2f &sample,
        ...
        // Sample point for angular spread
        Point2f angular = warp::square_to_uniform_disk(sample) * dr::sin(m_spread_angle); 

        Vector3f d = m_to_world.value().transform_affine(dr::normalize(Vector3f{ angular.x(), angular.y(), 1.f }));
    ...
    std::string to_string() const override {
        ...
        oss << "DirectionalSpreadEmitter[" << std::endl
            << "  irradiance = " << string::indent(m_irradiance) << ","
            << std::endl
            << "  spread_angle = " << string::indent(m_spread_angle) << ","
            << std::endl
...
protected:
    ...
    Float m_spread_angle;
};

MI_IMPLEMENT_CLASS_VARIANT(DirectionalSpreadEmitter, Emitter)
MI_EXPORT_PLUGIN(DirectionalSpreadEmitter, "Distant directional emitter with angular spread")
